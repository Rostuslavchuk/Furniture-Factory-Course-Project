document.addEventListener('DOMContentLoaded', (e) => {
    ...
    function modalUpdateAddErrors() {
        ...
        document.getElementById('err_inspection_area').textContent = '';
        document.getElementById('err_inspection_date').textContent = '';
        document.getElementById('err_issues_found').textContent = '';
        document.getElementById('err_corrective_action').textContent = '';
    ...
    function errorLog(code) {
        switch (code) {
            ...
            case 2:
                return 'err_position';
            case 3:
                return 'err_inspection_area'
            case 4:
                return 'err_inspection_date';
            case 5:
                return 'err_issues_found';
            case 6:
                return 'err_corrective_action';
            case 7:
                return 'err_salary';
            case 8:
    ...
    function add(data) {
        ...
        <tr data-id="${data.id}">
            ...
            <td class="inspection_area" >${data.inspection_area}</td>
            <td class="inspection_date" >${data.inspection_date}</td>
            <td class="issues_found" >${data.issues_found}</td>
            <td class="corrective_action" >${data.corrective_action}</td>
            <td class="salary" >${data.salary}</td>
            <td class="image_employee">
                <img src="./quality_control/QC${data.id}.jpg" style="width: 75px; height: 75px; object-fit: cover;" alt="Emploee Image">
            </td>
            <td>
    ...
    function update(data) {
        ...
        if (updateHtml) {
            ...
            updateHtml.querySelector('.inspection_area').textContent = data.inspection_area;
            updateHtml.querySelector('.inspection_date').textContent = data.inspection_date;
            updateHtml.querySelector('.issues_found').textContent = data.issues_found;
            updateHtml.querySelector('.corrective_action').textContent = data.corrective_action;
            ...
            if (imgElement) {
                ...
                imgElement.src = `./quality_control/QC${id_update}.jpg?timestamp=${timestamp}`;
            }
        }
    }
            ...
            btn.onclick = async (e) => {
                ...
                if(response.status) {
                    ...
                    if (getData) {
                        ...
                        addUpdateModalParams.form.elements['inspection_area'].value = getData.querySelector('.inspection_area').textContent;
                        addUpdateModalParams.form.elements['inspection_date'].value = getData.querySelector('.inspection_date').textContent;
                        addUpdateModalParams.form.elements['issues_found'].value = getData.querySelector('.issues_found').textContent;
                        addUpdateModalParams.form.elements['corrective_action'].value = getData.querySelector('.corrective_action').textContent;
    ...
    async function data(data) {
        const url = "http://localhost:63342/furniture_factory/view/mainPage/workStaff/quality_control/validate.php";
